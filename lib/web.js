...
function buildBidQuery(req) {
    var query = {$text: {$search: striptags(req.body.search), $options: 'i'}};
    if (req.body.site) {
...
app.get('/bids/totals', function(req, res) {
    var open = auctions.count({'state': 'open'})
    var close = auctions.count({'state': 'closed'})
...
app.post('/bids/search/stats', function(req, res){
    ...
    function reduce(key, values) {
        ...
        a.avg = a.sum / a.count;
        a.variance = a.diff / a.count;
        a.stddev = Math.sqrt(a.variance);
        return a;
    }
