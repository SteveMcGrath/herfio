...
function buildBidQuery(req) {
    ...
    if (req.body.sites) {
        query.site = {$in: striptags(req.body.sites).split(',')};
    }
    ...
    if (req.body.types) {
        query.type = {$in: striptags(req.body.types).split(',')};
    } else {
        if (!req.body.exact) {
...
app.get('/bids', function(req, res) {
    res.render('bidhistory', {user: req.user})
})
...
// POST /bids/search/totals
//   Get the count of open and closed auctions based on the serch criteria.
app.post('/bids/search/totals', function(req, res) {
    var query = buildBidQuery(req);
    query.state = 'open'
    auctions.count(query).then(function(openCount) {
        query.state = 'closed'
        auctions.count(query).then(function(closedCount) {
            return res.send({
                open: openCount,
                closed: closedCount
            })
...
app.post('/bids/search/closed', function(req, res) {
    var query = buildBidQuery(req);
    query.state = 'closed'
    auctions.find(query, {sort: {closed: -1}}).then(function(docs) {
        res.send(docs)
    })
})
...
// POST /bids/search/distribution
//   Returns the price distribution
app.post('/bids/search/distribution', function(req, res) {
    var stats = new Stats({bucket_precision: 25});
    var query = buildBidQuery(req);
    query.price = {$ne: null};
    query.state = 'closed';
    auctions.find(query, {fields: {price: 1, quantity: 1}}).each(function (doc){
        stats.push((doc.price / doc.quantity) * 100);
    }).then(function() {
        var results = [];
        stats.distribution().forEach(function(item) {
            results.push([item.bucket / 100, item.count]);
        })
        res.send(results);
    })
})
...
app.post('/bids/search/timeline', function(req, res) {
    ...
    query.price = {$ne: null};
    query.state = 'closed';
    auctions.find(query, {fields: {price: 1, quantity: 1, closed: 1, _id: -1}}).each(function(doc) {
        timeline.push([doc.closed, roundToTwo((doc.price / doc.quantity))]);
    }).then(function() {
...
app.post('/bids/search/stats', function(req, res){
    ...
    query.state = 'closed';
    auctions.find(query, {fields: {price: 1, quantity: 1}}).each(function (doc) {
        stats.push((doc.price / doc.quantity));
    }).then(function() {
        return res.send({
...
//   Returns the list of humidors that the user has created.
...
// GET /humidors/info/:id
//   NOTE: authentication required
//   Returns specific information about the humidor specified.
...
// POST /humidrs/info/new
//   NOTE: authentication required
//   Creates a new humidor
...
// POST /humidors/info/:id
//   NOTE: authentication required
//   Updates an existing humidor
...
// POST /humidors/update
//   Tempurature and humidity trackers will use this URL to post new readings here.
//   Once we have these readings we will also fire off any potential alarms as needed.
app.post('/humidors/update', function(req, res) {
    if (req.body.humidor && req.body.temperature && req.body.humidity){
        ...
        humidors.findAndModify({
            ...
            console.log(err)
            res.send({status: 'error', message: 'database issue'})
        })
    } else {
        res.send({status: 'error', message: 'humidor, temperature, and humidity must all be specified'})
    }
})
